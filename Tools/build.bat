...
echo Launching: Tools\build.bat %*
...
if "%2"=="" goto argend
set ARG=%2
if "%ARG:~0,1%"=="/" set BUILD_COMMAND=%ARG%& set BUILD_COMMAND_ARG=%ARG:~1%& goto :argloop
...
set TARGET_VS_CONFIG_MSBUILD=/p:Configuration="%TARGET_PROJ_CONFIG% %BUILD_FLAVOR%" /p:platform=%BUILD_ARCH%
...
if /I "!TAG!"=="SDV" (
  ...
  rem call "C:\Program Files (x86)\Microsoft Visual Studio\2017\!VSFLAVOR!\Common7\IDE\devenv.com" %BUILD_FILE% %BUILD_COMMAND% %TARGET_VS_CONFIG% /Out %BUILD_LOG_FILE%
  call MSBuild.exe "%BUILD_FILE%" /t:%BUILD_COMMAND_ARG% %TARGET_VS_CONFIG_MSBUILD% /m /v:n
)
popd
endlocal
...
msbuild.exe %BUILD_FILE% /t:clean /p:Configuration="%~1" /P:Platform=%2 /m /v:n

IF ERRORLEVEL 1 (
  set BUILD_FAILED=1
)

msbuild.exe %BUILD_FILE% /t:sdv /p:inputs="/clean" /p:Configuration="%~1" /P:platform=%2 /m /v:n

IF ERRORLEVEL 1 (
  set BUILD_FAILED=1
)

msbuild.exe %BUILD_FILE% /p:Configuration="%~1" /P:Platform=%2 /P:RunCodeAnalysisOnce=True /m /v:n

IF ERRORLEVEL 1 (
  set BUILD_FAILED=1
)

msbuild.exe %BUILD_FILE% /t:sdv /p:inputs="/check /devenv" /p:Configuration="%~1" /P:platform=%2 /m /v:n

IF ERRORLEVEL 1 (
  set BUILD_FAILED=1
)

msbuild.exe %BUILD_FILE% /t:dvl /p:Configuration="%~1" /P:platform=%2 /m /v:n
